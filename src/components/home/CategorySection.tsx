  ...
  return (
    <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="sm:hidden flex flex-col gap-3">
        {/* First card — wide banner */}
        <Link
          href={categoryConfig[0].href}
          className="group relative overflow-hidden rounded-2xl aspect-[16/9] shadow-sm active:scale-[0.98] transition-transform duration-200"
        >
          <img
        ...
        <div className="grid grid-cols-2 gap-3">
          {categoryConfig.slice(1).map((cat) => (
            <Link
              key={cat.name}
              href={cat.href}
              className="group relative overflow-hidden rounded-2xl aspect-square shadow-sm active:scale-[0.98] transition-transform duration-200"
            >
              <img
                src={cat.image}
                alt={cat.name}
      ...
      <div className="hidden sm:grid sm:grid-cols-3 gap-4">
        {categoryConfig.map((cat) => (
          <Link
            key={cat.name}
            href={cat.href}
            className="group relative overflow-hidden rounded-2xl aspect-[3/4] shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
          >
            <img
              src={cat.image}
              alt={cat.name}
